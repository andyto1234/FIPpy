...
def plot_ionization(results):
    ...
    fig.suptitle(f'Ionization Fractions Along the Coronal Loop\nFrequency: {results["frequency"]:.3f}, Amplitude: {results["amplitude"]:.1f}, Loop Length: {results["loop_length"].to(u.Mm):.1f}', fontsize=12)
...
def process_hydrad_data(input_file, frequency, amplitude, strand_num=105):
    ...
    for element in elements:
        ...
        plot_filename = f'fip_bias_profile_{element}_f{frequency.value:.3f}_A{amplitude.value:.1f}.png'
    ...
    colors = {'Ca': 'blue', 'Si': 'red', 'Fe': 'orange', 'S': 'green', 'O': 'purple', 'Ar': 'gray', 'Mg': 'pink'}
    for element in elements:
        # Plot left side
    ...
    # Save plot
    output_dir = '/Users/andysh.to/Script/Python_Script/alfven_loop_SM/test_data/output/'
    plt.savefig(output_dir + 'fip_bias_vs_height_multi.png', dpi=300, bbox_inches='tight')
    plt.close()
    # import pdb; pdb.set_trace()
    # ------------------------------------------------------------
    ...
    full_loop_plot_path = os.path.join(output_dir, f'full_loop_plot_f{output_data["frequency"].value:.3f}_A{output_data["amplitude"].value:.1f}_L{output_data["loop_length"].to(u.Mm).value:.1f}.png')
...
def save_results(output_data, output_file):
    ...
    chromosphere_A_plot_path = os.path.join(output_dir, f'chromosphere_A_f{output_data["frequency"].value:.3f}_A{output_data["amplitude"].value:.1f}_L{output_data["loop_length"].to(u.Mm).value:.1f}.png')
    with open(chromosphere_A_plot_path, 'wb') as f:
        f.write(chromosphere_A_plot)

    chromosphere_B_plot = plot_chromosphere(output_data, 'B')
    chromosphere_B_plot_path = os.path.join(output_dir, f'chromosphere_B_f{output_data["frequency"].value:.3f}_A{output_data["amplitude"].value:.1f}_L{output_data["loop_length"].to(u.Mm).value:.1f}.png')
...
def main(input_file, frequencies, amplitudes, output_dir, strand_num):
    for frequency in frequencies:
        for amplitude in amplitudes:
            results = process_hydrad_data(input_file, frequency, amplitude, strand_num)
            output_file = os.path.join(output_dir, f'output_f{frequency.value:.3f}_A{amplitude.value:.1f}_L{results["loop_length"].to(u.Mm).value:.1f}.pkl')
...
def plot_full_loop(results):
    ...
    fig.suptitle(f'Wave and Plasma Properties Along the Full Coronal Loop\nFrequency: {results["frequency"]:.3f}, Amplitude: {results["amplitude"]:.1f}, Loop Length: {results["loop_length"].to(u.Mm):.1f}', fontsize=16)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 4:  # Changed to 3 since we need 2 arguments
    ...
    # Define the frequencies and amplitudes to run
    # frequencies = [0.017,0.02,0.023, 0.025,0.027] * u.s**-1
    # frequencies = [0.1,0.2] * u.s**-1
    # frequencies = [0.2] * u.s**-1
    # frequencies = [0.1] * u.s**-1 # resonance-ish
    frequencies = [0.94] * u.s**-1
    amplitudes = [0.25] * u.km/u.s 
    # frequencies = [0.025] * u.s**-1
    # amplitudes = [0.03] * u.km/u.s
    
    main(input_file, frequencies, amplitudes, output_dir, strand_num)

# Example run:
# python main_fip_bias.py run_20241017_171935/val_c_initial.pkl run_20241017_171935/output


# python main_fip_bias.py /Users/andysh.to/Script/Python_Script/fip_bias_hydrad/hydards/HYDRAD_flare_LL6e9_he1.8e8x5_SI_7_CF_5_train_with_H /Users/andysh.to/Script/Python_Script/fip_bias_hydrad/hydards/HYDRAD_flare_LL6e9_he1.8e8x5_SI_7_CF_5_train_with_H/output 0